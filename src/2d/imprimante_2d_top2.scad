...
module top_bottom(){
    difference(){
        ...
        for(i = [0,1]){
            mirror([0,i,0]) translate([-l/2, -l_top/2 -1]) cran_x_nuts(l, e + 1, cran, vis_fixation, vis_emplacement);
        }
            
        for(i = [0,1]){
            mirror([i,0,0]) translate([-l/2 -1, -l_top/2])
                cran_y_nuts(l_top, e + 1, cran, vis_fixation, vis_emplacement);
        }
    }
}
...
module back(){
    difference(){
        ...
        for(i = [0,1]){
            mirror([i,0,0]) translate([-l/2-1, 0]) cran_y_nuts(h, e + 1, cran, vis_fixation, vis_emplacement);
        }
        
        translate([-l/2, h2-e]) cran_x_vis(l, e, cran, vis_fixation, vis_emplacement);
    }
}
...
module front(){
    difference(){
        ...
        h_top = max(0, h_front-top_int/2);
        hull(){
            translate([-top_int/2, h_top])circle(top_int_r);
            translate([+top_int/2, h_top])circle(top_int_r);
...
module left_right(){
    difference(){
        ...
        translate([-lg/2,h2]) rotate([0,0, -90]) cran_y_vis(l_top, e, cran, vis_fixation, vis_emplacement);
        translate([-lg/2,0]) cran_y_vis(h, e, cran, vis_fixation, vis_emplacement);
        translate([lg/2,0])  rotate([0,0, angle]) translate([-e,0])cran_y_vis(h_front, e, cran, vis_fixation, vis_emplacement);
...
module imprimante_3d_top2(e2 = e){
...
if(false){
    imprimante_3d_top2(1);
} else {
    e= 10;
    translate([-l/2, 0])plaque(456, 609);
    translate([0,20])left();
    translate([0,h+42])right();
    translate([0,2*h+64])back();
    
    translate([-(456+10),0]){
        translate([-l/2, 0]) plaque(456, 609);
        translate([0, lg+22]) front();
